...
function hasEsmSyntax(filePath) {
  try {
    const source = readFileSync(filePath, "utf8");
    return /^\s*(?:import|export)\s/m.test(source) || /\bexport\s*\{/.test(source);
  } catch {
    return false;
  }
}

function isModuleLikeFile(manifest, filePath) {
  return manifest.type === "module" || path.extname(filePath) === ".mjs" || hasEsmSyntax(filePath);
}

function resolveExportsEntry(manifest) {
  try {
...
function resolveLegacyEntry(manifest) {
  try {
...
function findSourceEntry(packageName, packageDir, manifest) {
...
function buildPackageUrl(packageName, version, entry, isProd) {
  if (isProd) {
...
function buildCurrentPackageUrl(isProd, entry) {
  if (!isProd) {
...
export function getPackageImports(isProd) {
